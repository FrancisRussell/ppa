...
# debian/rules expects the binaries in target/release/; symlink the
# cross-compiled output. Remove target/release first: cargo creates it as a
# directory even for --target builds, and ln -sf into an existing directory
# would place the symlink inside it rather than replacing it.
